...
/* AuthScript primitives against the JS oracle vectors in tmp/oracle/vectors.json.
 * Constants below are the values that script produces for the canonical mnemonic:
 *   "result pact model attract result puzzle final boss private educate luggage era"
 * on network "xna-pq-test", path m_pq/100'/1'/0'/0'/0'.
 */
...
/* PQ public key for m_pq/100'/1'/0'/0'/0' — 1312 raw bytes (2624 hex chars). */
static const char PQ_PUBKEY_HEX[] =
...
static const char EXPECTED_AUTHDESC[] = "014f193423a3d4bf9f3a99855481c794da113765ee";
static const char EXPECTED_COMMITMENT[] =
    "6c24fe896c439911b91bfc74f82c240a94710d08223ba9d60dccf795ef4e6456";

/* OP_TRUE — the placeholder witnessScript used everywhere in phase 1. */
static const uint8_t WITNESS_SCRIPT[] = { 0x51 };

MU_TEST(test_tagged_hash_known_input){
    /* Compare against a value computed independently in the oracle script
     * (and easily reproducible in any hashing tool):
     *   taggedHash("NeuraiAuthScript", "") = SHA256(SHA256(tag)||SHA256(tag)) */
    uint8_t out[32];
    taggedHashAuthScript(NULL, 0, out);

    /* SHA256("NeuraiAuthScript") then SHA256(t || t). */
    uint8_t tagHash[32];
    sha256((const uint8_t *)UNEURAI_AUTHSCRIPT_TAG,
           strlen(UNEURAI_AUTHSCRIPT_TAG), tagHash);
    SHA256 h;
    h.begin();
    h.write(tagHash, 32);
    h.write(tagHash, 32);
    ...
    mu_check(memcmp(out, expected, 32) == 0);
}

MU_TEST(test_auth_descriptor_noauth){
    uint8_t out[22] = {0xff};
    size_t n = buildAuthDescriptor(UNEURAI_AUTHTYPE_NOAUTH, NULL, 0, out, sizeof(out));
    mu_assert_int_eq(1, (int)n);
    mu_assert_int_eq(0x00, out[0]);
}

MU_TEST(test_auth_descriptor_pq_matches_oracle){
    ...
    uint8_t desc[22];
    size_t n = buildAuthDescriptor(UNEURAI_AUTHTYPE_PQ, pk, pkLen, desc, sizeof(desc));
    mu_assert_int_eq(21, (int)n);

    string got = toHex(desc, n);
    mu_assert_string_eq(EXPECTED_AUTHDESC, got.c_str());
}

MU_TEST(test_auth_descriptor_pq_wrong_length_rejected){
    uint8_t pk[100] = {0};
    uint8_t desc[22];
    size_t n = buildAuthDescriptor(UNEURAI_AUTHTYPE_PQ, pk, sizeof(pk), desc, sizeof(desc));
    mu_assert_int_eq(0, (int)n);
}

MU_TEST(test_commitment_matches_oracle){
    uint8_t pk[UNEURAI_PQ_PUBKEY_RAW_LEN];
    size_t pkLen = fromHex(PQ_PUBKEY_HEX, sizeof(PQ_PUBKEY_HEX) - 1, pk, sizeof(pk));
    mu_assert_int_eq(UNEURAI_PQ_PUBKEY_RAW_LEN, (int)pkLen);

    uint8_t commitment[32];
    int ok = buildAuthScriptCommitment(UNEURAI_AUTHTYPE_PQ, pk, pkLen,
                                       WITNESS_SCRIPT, sizeof(WITNESS_SCRIPT),
                                       commitment);
...
MU_TEST(test_scriptpubkey_shape){
    uint8_t commitment[32];
    for(int i = 0; i < 32; i++) commitment[i] = (uint8_t)(i + 0x11);

    uint8_t spk[40] = {0};
    size_t n = buildAuthScriptScriptPubKey(commitment, spk, sizeof(spk));
    mu_assert_int_eq(34, (int)n);
    mu_assert_int_eq(0x51, spk[0]);
...
MU_TEST_SUITE(test_suite){
    ...
    MU_RUN_TEST(test_auth_descriptor_pq_matches_oracle);
